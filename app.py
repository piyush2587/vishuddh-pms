...
if st.sidebar.button("📂 Load Watchlist"):
    if os.path.exists(watchlist_file):
        ...
        st.sidebar.warning("No watchlist found.")
...
for ticker in stocks:
    ...
    # Chart
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df.loc[y_test.index, 'Date'], y=y_test, name="Actual"))
    fig.add_trace(go.Scatter(x=df.loc[y_test.index, 'Date'], y=y_pred, name="Predicted"))
    ...
    try:
        ...
        try:
            ...
            st.warning(f"AI Insight error for {ticker}: {e}")
    ...
    try:
        ...
        st.warning(f"Excel data error: {e}")
    ...
    if st.button(f"📄 Download PDF Report for {ticker}"):
        try:
            ...
            pdf.ln(10)
            pdf.multi_cell(0, 10, "Note: This report is generated using AI. Please use your own judgment when trading.")
            pdf_output = BytesIO()
...
# --- Excel Export for All Predictions ---
if all_predictions:
    try:
