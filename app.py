...
for ticker in stocks:
    ...
    # Build chart only if prediction output is valid
    try:
    ...
    try:
        ...
        features = np.array([
            [len(df),
             float(last_row['7MA']) if not pd.isna(last_row['7MA']) else 0.0,
             float(last_row['30MA']) if not pd.isna(last_row['30MA']) else 0.0]
        ])
